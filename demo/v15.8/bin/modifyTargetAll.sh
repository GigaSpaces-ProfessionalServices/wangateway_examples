...
################################################################################
# This script gets the spaces are available and will call another script to add
# or remove outbound gateway targets.
################################################################################
...
echo
echo "Running the modify target for all spaces..."
...
for i in "${SPACES[@]}"; do
  echo
  echo "Deploying modify target for $i"
  $BIN_DIR/modifyTargetHelper.sh "$DC" "$CLUSTER" "$i"
done
