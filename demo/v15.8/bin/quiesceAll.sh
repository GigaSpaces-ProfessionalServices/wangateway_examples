...
################################################################################
# This script is now using curl with REST Manager to quiesce.
...
echo
echo "Running quiesce for all spaces..."
...
for SPACE in "${SPACES[@]}"; do
  ...
  echo
  echo "Quiescing $SERVICE_NAME..."
  echo "Press Y/ENTER to quiesce: "
  read INPUT_VAR;
  if [ -z "$INPUT_VAR" ] || [ "Y" = "$INPUT_VAR"  ]; then
    #$GS_CMD service quiesce "$SERVICE_NAME"
    REQUEST_ID=$(quiesce_pu "$REST_HOST" "$REST_PASSWORD_FILE" "$SERVICE_NAME")
    echo "Request ID: $REQUEST_ID was created."
  else
    echo "Skipping quiesce for $SERVICE_NAME";
  fi
done
