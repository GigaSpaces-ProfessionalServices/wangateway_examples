...
################################################################################
# admin functions using the REST Manager
################################################################################
# Where defined, 
...
function quiesce_pu() {
  ...
  REQUEST_ID=$(curl -s -X POST --header 'Content-Type: application/json' --header 'Accept: text/plain' --netrc-file $PASSWORD_FILE "http://$REST_HOST/v2/pus/$PU_NAME/quiesce")
  echo "$REQUEST_ID"
}


function unquiesce_pu() {
  REST_HOST="$1"
  PASSWORD_FILE="$2"
  PU_NAME="$3"

  REQUEST_ID=$(curl -s -X POST --header 'Content-Type: application/json' --header 'Accept: text/plain' --netrc-file $PASSWORD_FILE "http://$REST_HOST/v2/pus/$PU_NAME/unquiesce")
  echo "$REQUEST_ID"
}

function space_type_counts() {
  REST_HOST="$1"
  PASSWORD_FILE="$2"
  SPACE_NAME="$3"

  REQUEST_INFO=$(curl -s -X GET --header 'Accept: application/json' --netrc-file $PASSWORD_FILE "http://$REST_HOST/v2/spaces/$SPACE_NAME/statistics/types")
  echo "$REQUEST_INFO"
}
